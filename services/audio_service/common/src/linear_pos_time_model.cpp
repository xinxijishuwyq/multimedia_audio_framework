...
int64_t LinearPosTimeModel::GetTimeOfPos(uint64_t posInFrame)
{
    ...
    if (posInFrame >= stampFrame_) {
        CHECK_AND_RETURN_RET_LOG((posInFrame - stampFrame_ < sampleRate_), invalidTime, "invalid frame, too large");
        deltaFrame = posInFrame - stampFrame_;
        return stampNanoTime_ + deltaFrame * NANO_COUNT_PER_SECOND / (int64_t)sampleRate_;
    } else {
        CHECK_AND_RETURN_RET_LOG((stampFrame_ - posInFrame < sampleRate_), invalidTime, "invalid frame, too small");
