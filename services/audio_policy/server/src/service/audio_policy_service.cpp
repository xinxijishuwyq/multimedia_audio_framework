...
{
    AUDIO_DEBUG_LOG("Entered %{public}s size: %{public}d", __func__, outputPriorityList_.size());
    DeviceType priorityDevice = isOutputDevice ? DEVICE_TYPE_SPEAKER : DEVICE_TYPE_MIC;
    AUDIO_INFO_LOG("priorityDevice: %{public}d currentActiveDevice: %{public}d", priorityDevice, currentActiveDevice_);
    ...
    AUDIO_INFO_LOG("FetchHighPriorityDevice return priorityDevice: %{public}d", priorityDevice);
...
int32_t AudioPolicyService::SetAudioScene(AudioScene audioScene)
{
    ...
    if ((audioScene_ == AUDIO_SCENE_PHONE_CALL || audioScene_ == AUDIO_SCENE_PHONE_CHAT)) {
        if (find(outputPriorityList_.begin(), outputPriorityList_.end(), DEVICE_TYPE_EARPIECE)
            == outputPriorityList_.end()) {
            outputPriorityList_.insert(outputPriorityList_.end() - PRIORITY_LIST_OFFSET_POSTION, DEVICE_TYPE_EARPIECE);
        }
    } else {
        std::vector<DeviceType>::iterator earpiecePos = find(outputPriorityList_.begin(),
            outputPriorityList_.end(), DEVICE_TYPE_EARPIECE);
...
InternalDeviceType AudioPolicyService::GetDeviceType(const std::string &deviceName)
{
    ...
    } else if (deviceName == "Earpiece") {
        devType = InternalDeviceType::DEVICE_TYPE_EARPIECE;
    ...
    AUDIO_INFO_LOG("AudioPolicyService::GetDeviceType devType: %{public}d", devType);
    return devType;
}
