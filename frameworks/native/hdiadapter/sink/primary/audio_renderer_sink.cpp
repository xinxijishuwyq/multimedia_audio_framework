...
int32_t AudioRendererSinkInner::UpdateDPAttrs(const std::string &usbInfoStr)
{
    ...
    if (attr_.channel <= 0 || attr_.sampleRate <= 0 || BUFFER_CALC_20MS <= 0) {
        AUDIO_ERR_LOG("check attr failed channel[%{public}d] sampleRate[%{public}d] sampleRate[%{public}d]",
            attr_.channel, attr_.sampleRate, BUFFER_CALC_20MS);
    }
