...
static pa_hook_result_t SinkInputNewCb(pa_core *c, pa_sink_input *si)
{
    ...
    if (!pa_safe_streq(clientUid, bootUpMusic)) {
        ...
        if (si->state == PA_SINK_INPUT_RUNNING && EffectChainManagerAddSessionInfo(
            sceneType, sessionID, channels, channelLayout, sceneMode, spatializationEnabled)) {
...
static pa_hook_result_t SinkInputStateChangedCb(pa_core *c, pa_sink_input *si)
{
    ...
    if (si->state == PA_SINK_INPUT_RUNNING && si->sink && !pa_safe_streq(clientUid, bootUpMusic)) {
        if (EffectChainManagerAddSessionInfo(
            sceneType, sessionID, channels, channelLayout, sceneMode, spatializationEnabled)) {
            EffectChainManagerMultichannelUpdate(sceneType);
        }
    }
