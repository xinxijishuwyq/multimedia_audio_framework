...
void AudioRendererCallbackNapi::RemoveCallbackReference(const std::string &callbackName)
{
    std::lock_guard<std::mutex> lock(mutex_);
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    AUDIO_DEBUG_LOG("OnInterrupt is called");
    AUDIO_DEBUG_LOG("hintType: %{public}d", interruptEvent.hintType);
...
{
    std::lock_guard<std::mutex> lock(mutex_);
    AUDIO_DEBUG_LOG("OnStateChange is called");
    AUDIO_DEBUG_LOG("state: %{public}d", state);
...
void AudioRendererCallbackNapi::OnJsCallbackStateChange(std::unique_ptr<AudioRendererJsCallback> &jsCb)
{
    ...
    uv_work_t *work = new(std::nothrow) uv_work_t;
    if (work == nullptr) {
        AUDIO_ERR_LOG("OnJsCallbackStateChange: No memory");
        return;
    }
    if (jsCb.get() == nullptr) {
