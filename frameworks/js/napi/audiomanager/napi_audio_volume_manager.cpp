...
napi_value NapiAudioVolumeManager::Off(napi_env env, napi_callback_info info)
{
    ...
    return UnregisterCallback(env, jsThis, argCount, args, callbackName);
}

napi_value NapiAudioVolumeManager::UnregisterCallback(napi_env env, napi_value jsThis, size_t argc, napi_value *args,
    const std::string &cbName)
{
    napi_value undefinedResult = nullptr;
    NapiAudioVolumeManager *napiVolumeManager = nullptr;
    ...
    if (!cbName.compare(VOLUME_KEY_EVENT_CALLBACK_NAME)) {
        if (napiVolumeManager->volumeKeyEventCallbackNapi_ != nullptr) {
            napiVolumeManager->audioSystemMngr_->UnregisterVolumeKeyEventCallback(
